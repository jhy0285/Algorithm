...
위의 코드에서, 처음에 main에서 순열함수를 실행하면 permu의 현재순서(cnt==0)에 들어갈숫자를 찾기위해 반복문에서 100을 먼저 검사한다.

100은 방문하지 않았으므로 현재순서(0번쨰)에 100을 넣는다

그러면 [100,]이렇게 permu에 저장이 되어있을것이고, 100이라는 숫자는 사용되었으므로 visited[0]을 true로 바꿔준다.

그런다음 permu의 다음순서(cnt+1==1)의 숫자를 정하기위해 다시 순열함수로 들어간다.

그럼 cnt+1번쨰 함수에서 반복문은 다시 100부터 permu의 cnt+1번쨰에 넣으려고 할텐데, 이때 100은 방문한적이있기 떄문에(visited[0]==true)
아무 행위도 하지않고 그다음 200을 넣으려고 할것이다. 

200은 아직 방문안했으므로 cnt+1번쨰 넣을수있고 permu는 [100,200]이 된다. 그런다음 200은 사용했으니 visited[1]을 true로 바꿔준다.

이제 permu의 (cnt+2==2)번쨰에 넣을 숫자를 찾으러 순열함수로 들어간다. 

cnt+2번쨰 순열함수에서는 cnt==R 조건식에 걸리므로 permu를 answer에 저장하고 재귀함수를 나간다.

나온 재귀함수는 현재 cnt+1번쨰 재귀함수이고, cnt+1번째에서는 200의 사용이 끝났으므로(즉 cnt+1번쨰가 200인 모든 경우의수를 재귀적으로 탐방하고 왔으므로) visited[1]=false로 바꾼다.
(다음 재귀 탐방떄 재사용하기 위해서 방문을 초기화한다.)

그런 다음 300을 permu의 cnt+1번째에 넣으려고 할것이다.

300은 방문하지 않았으므로 cnt+1번쨰 넣을수 있고 permu는 [100,300]이 된다 그런다음 300은 사용했으니 visited[2]를 true로 바꿔준다

이제 cnt+2번쨰 순열함수에서는 cnt==R 조건식에 걸리므로 permu를 answer에 저장하고 재귀함수를 나간다.

나온 재귀함수는 현재 cnt+1번쨰 재귀함수이고, cnt+1번째에서는 300의 사용이 끝났으므로 visited[2]=false로 바꾼다.

이렇게 400까지 반복을하고 cnt+1번쨰 재귀함수의 반복문이 끝나면 cnt번쨰 재귀함수로 돌아간다.

cnt번째에서는 100의 사용이 끝났으므로(즉 cnt번쨰가 100인 모든 경우의수를 재귀적으로 탐방하고 왔으므로) visited[0]=false로 바꾼다.
(다음 재귀 탐방떄 재사용하기 위해서 방문을 초기화한다.)

이제 cnt번쨰함수의 반복문에서 100다음인 200을 permu의 cnt번째에 넣으려고 할텐데, 앞서 cnt번쨰가 100일때가 끝나면서 모든 visited가 false가 되었으므로
